# THIS FILE CONSTRUCTS A U-Net like network, uses dice loss, data augmentation, test time augmentation, prediction on 9 patches
...
def main():
    ...
    #validation_dataset = list_ds.take(NUM_VALIDATION_IMAGES).map(lambda x: process_path(x, "images_rotated", "groundtruth_rotated"), num_parallel_calls=AUTOTUNE).map(resize_validation).map(augment_validation).batch(BATCH_SIZE).repeat()
    ...
    #test_dataset = test_list_ds.map(test_process_path).map(crop_test_0)
    ...
    #conv5 = tf.keras.layers.BatchNormalization() (conv5)
    conv5_1 = tf.keras.layers.Conv2D(256, (3, 3), activation='relu', dilation_rate=1, padding='same') (pooling4)
    conv5_2 = tf.keras.layers.BatchNormalization() (conv5_1)
    conv5_2 = tf.keras.layers.Dropout(0.05) (conv5_2)
    conv5_2 = tf.keras.layers.Conv2D(256, (3, 3), activation='relu', dilation_rate=2, padding='same') (conv5_2)
    conv5_4 = tf.keras.layers.BatchNormalization() (conv5_2)
    conv5_4 = tf.keras.layers.Dropout(0.05) (conv5_4)
    conv5_4 = tf.keras.layers.Conv2D(256, (3, 3), activation='relu', dilation_rate=4, padding='same') (conv5_4)
    conv5_8 = tf.keras.layers.BatchNormalization() (conv5_4)
    conv5_8 = tf.keras.layers.Dropout(0.05) (conv5_8)
    conv5_8 = tf.keras.layers.Conv2D(256, (3, 3), activation='relu', dilation_rate=8, padding='same') (conv5_8)
    conv5_16 = tf.keras.layers.BatchNormalization() (conv5_8)
    conv5_16 = tf.keras.layers.Dropout(0.05) (conv5_16)
    conv5_16 = tf.keras.layers.Conv2D(256, (3, 3), activation='relu', dilation_rate=16, padding='same') (conv5_16)
    conv5_32 = tf.keras.layers.BatchNormalization() (conv5_16)
    conv5_32 = tf.keras.layers.Dropout(0.05) (conv5_32)
    conv5_32 = tf.keras.layers.Conv2D(256, (3, 3), activation='relu', dilation_rate=32, padding='same') (conv5_32)
