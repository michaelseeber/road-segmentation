...
def main():
    ...
    precision = tf.keras.metrics.Precision(
        name='precision', dtype=None, thresholds=0.0
    )
    recall = tf.keras.metrics.Recall(
        name='recall', dtype=None, thresholds=0.0
    )

    model.compile(optimizer='adam',
                  #loss=soft_dice_loss,
                  loss=lovasz_hinge,
                  metrics=[accuracy, precision, recall, F1Score_with_logits(), meanIoU_with_logits(num_classes=2)])
