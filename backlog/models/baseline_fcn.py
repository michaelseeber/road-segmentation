# THIS FILE CONSTRUCTS A SIMPLE FCN, used as a baseline, without augmentation and hyperparameter tuning
...
def main():
    ...
    validation_dataset = list_ds.take(NUM_VALIDATION_IMAGES).map(lambda x: process_path(x, "images", "groundtruth"), num_parallel_calls=AUTOTUNE).map(resize_validation).batch(BATCH_SIZE).repeat()
    ...
    model.compile(optimizer='adam',
                  loss=soft_dice_loss,
                  #metrics=['accuracy', F1Score_with_probabilities(), meanIoU_with_probabilities(num_classes=2)])
                  metrics=['accuracy', tf.keras.metrics.Precision(), meanIoU_with_probabilities(num_classes=2)])
