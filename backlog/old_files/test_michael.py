...
def categorical_crossentropy(y_true, y_pred):
    ...
    with tf.name_scope("categorical_cross_entropy"):
        y_true = tf.clip_by_value(y_true, tf.keras.backend.epsilon(), 1.0 - tf.keras.backend.epsilon())
        y_pred = tf.clip_by_value(y_pred, tf.keras.backend.epsilon(), 1.0 - tf.keras.backend.epsilon())

        # Measure how close we are to unknown class.
        # unkown_weights = 1 - y_true[..., -2][..., None]
        freespace_mask = tf.equal(tf.argmax(y_true, axis=-1), 0) #freespace
        street_mask  = ~freespace_mask
        ...
        freespace_cross_entropy = \
            -tf.reduce_sum(tf.boolean_mask(cross_entropy, freespace_mask)) / \
             (tf.reduce_sum(tf.cast(freespace_mask, tf.float32)) + tf.keras.backend.epsilon())

        street_cross_entropy = \
            -tf.reduce_sum(tf.boolean_mask(cross_entropy, street_mask)) / \
             (tf.reduce_sum(tf.cast(street_mask, tf.float32)) + tf.keras.backend.epsilon())

        #weight occupied space more
        cross_entropy = freespace_cross_entropy + bias* street_cross_entropy
