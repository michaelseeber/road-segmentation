...
test_list_ds = tf.data.Dataset.list_files('../test_images/test_images/*')
...
validation_dataset = list_ds.take(NUM_VALIDATION_IMAGES).map(process_path, num_parallel_calls=AUTOTUNE).batch(BATCH_SIZE).repeat()
test_dataset = test_list_ds.map(test_process_path, num_parallel_calls=AUTOTUNE).batch(BATCH_SIZE)
...
model.add(layers.Conv2D(filters=16, kernel_size=3, padding='same', activation='relu', input_shape=(IMG_WIDTH, IMG_HEIGHT, 3)))
...
model.add(layers.BatchNormalization())
model.add(layers.MaxPooling2D(pool_size=4, strides=4))
model.add(layers.Conv2DTranspose(filters=32, kernel_size=8, strides=4, padding='same'))
model.add(layers.Conv2DTranspose(filters=16, kernel_size=16, strides=8, padding='same'))
model.add(layers.Conv2DTranspose(filters=1, kernel_size=16, strides=8, padding='same'))
