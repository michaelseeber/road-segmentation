# THIS FILE CONSTRUCTS A smaller U-Net like network and predicts on 5x5 patches
...
def main():
    ...
    train_dataset = train.cache().shuffle(BUFFER_SIZE).map(lambda img, label: augment(img, label, horizontal_flip=False, random_crop=True, rotate90=False, color=False, block_noise=False), num_parallel_calls=AUTOTUNE).batch(BATCH_SIZE).repeat()
    train_dataset = train_dataset.prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
    validation_dataset = list_ds.take(NUM_VALIDATION_IMAGES).map(lambda x: process_path(x, "images", "groundtruth"), num_parallel_calls=AUTOTUNE).map(augment_validation_with_rand_crop).batch(BATCH_SIZE).repeat()
